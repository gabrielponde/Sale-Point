    ...
    async createProduct(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (!description || !category_id) {
                throw new ApiError('Description and category are required', 400);
            }

            const category = await this.productRepository.validateCategory(Number(category_id));
            ...
            const formattedProduct = this.productFormatter.formatProduct(product, category.id);
